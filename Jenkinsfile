    ...
    stages {
      ...
      stage ('defect dojo') {
        steps {
          ...
          curl -k -X POST "${DOJO_URL}/api/v2/import-scan/" -H Authorization:"Token ${DOJO_API_TOKEN}" -F scan_type="Trivy Scan" -F file=@trivy.json;type=application/json -F engagement=2 -H Content-Type:multipart/form-data -H accept:application/json -F "product_name=dronloko"
          '''
        }
      }      
  }
